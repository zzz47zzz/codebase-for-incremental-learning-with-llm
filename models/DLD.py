...
def get_DLD_params(parser):
    '''
        The parameters of model DLD
    '''
    parser.add_argument("--DLD_student_temperate", type=float, default=1, help="The temperature of student model")
    parser.add_argument("--DLD_teacher_temperate", type=float, default=1, help="The temperature of teacher model")
    parser.add_argument("--DLD_distill_weight", type=float, default=2, help="The weight of the distillation loss")
    parser.add_argument("--DLD_weight", type=float, default=2, help="The weight of the DLD loss")


class DLD(BaseLearner):
    '''
        a novel Decomposing Logits Distillation (DLD) method, enhancing the model's ability to retain old knowledge and mitigate catastrophic forgetting
        
        Decomposing Logits Distillation for Incremental Named Entity Recognition(2023 SIGIR)
    '''
    def __init__(self, params, CL_dataset, accelerator): 
        super().__init__(params, CL_dataset, accelerator)
        assert params.classifier in ['Linear','CosineLinear'], 'NotImplemented for classifier %s and model %s'%(params.classifier,'DLD')
        assert params.il_mode in ['CIL'], 'NotImplemented for il mode %s and model %s'%(params.il_mode,'DLD')
    ...
    def observe_batch(self, task_id, epoch_id, lm_input):
        ...
        if task_id == 0:
            ...
            # Emphasize the role of positive and negative terms
            DLD_positive_mask = (logits[:,:logits_teacher.shape[-1]]>0)
            DLD_nagetive_mask = (logits[:,:logits_teacher.shape[-1]]<0)
            ...
            if torch.sum(distill_mask).item()>0:
                distill_loss = self.distill_loss(F.log_softmax(logits[distill_mask]/self.params.DLD_student_temperate,dim=-1)[:,:logits_teacher.shape[-1]],
                                                F.softmax(logits_teacher[distill_mask]/self.params.DLD_teacher_temperate,dim=-1))
            else:
                distill_loss = torch.tensor(0.).to(logits.device)
                
            if torch.sum(DLD_positive_mask).item()>0:
                dld_positive_loss = self.distill_loss(F.log_softmax(logits[:,:logits_teacher.shape[-1]][DLD_positive_mask]/self.params.DLD_student_temperate,dim=-1),
                                                F.softmax(logits_teacher[DLD_positive_mask]/self.params.DLD_teacher_temperate,dim=-1))
            else:
                dld_positive_loss = torch.tensor(0.).to(logits.device)
            
            if torch.sum(DLD_nagetive_mask).item()>0:
                dld_nagetive_loss = self.distill_loss(F.log_softmax(logits[:,:logits_teacher.shape[-1]][DLD_nagetive_mask]/self.params.DLD_student_temperate,dim=-1),
                                                F.softmax(logits_teacher[DLD_nagetive_mask]/self.params.DLD_teacher_temperate,dim=-1))
            else:
                dld_nagetive_loss = torch.tensor(0.).to(logits.device)

            total_loss = ce_loss + self.params.DLD_distill_weight*distill_loss + self.params.DLD_weight*(dld_positive_loss + dld_nagetive_loss)
