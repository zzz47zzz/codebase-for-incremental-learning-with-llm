...
def get_SelfTrain_params(parser):
    '''
        The parameters of model SelfTrain
    '''
    parser.add_argument("--SelfTrain_student_temperate", type=float, default=1, help="The temperature of student model")
    parser.add_argument("--SelfTrain_distill_weight", type=float, default=1, help="The weight of the distillation loss")


class SelfTrain(BaseLearner):
    '''
        SelfTrain: Self Training is the same as knowledge distillation except that the teacher model provides hard pseudo labels instead of soft labels.

    '''
    def __init__(self, params, CL_dataset, accelerator): 
        super().__init__(params, CL_dataset, accelerator)
        assert params.classifier in ['Linear','CosineLinear'], 'NotImplemented for classifier %s and model %s'%(params.classifier,'SelfTrain')
        assert params.il_mode in ['CIL'], 'NotImplemented for il mode %s and model %s'%(params.il_mode,'SelfTrain')
    ...
    def observe_batch(self, task_id, epoch_id, lm_input):
        ...
        if task_id == 0:
            ...
            # Replace the 'O' label to the hard pseudo label provided by the teacher model 
            label_idx[label_idx==0] = torch.argmax(logits_teacher[label_idx==0],dim=-1)
            total_loss = self.ce_loss(logits,label_idx)
